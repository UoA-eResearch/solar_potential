...
def get_point():
    ...
    sql = """SELECT *, ST_ASTEXT(ST_TRANSFORM(the_geom, '+init=epsg:2193', '+init=epsg:3857')) AS the_geom FROM solar_potential WHERE ST_CONTAINS(the_geom, ST_TRANSFORM(ST_POINT(%s, %s), '+init=epsg:3857', '+init=epsg:2193'));"""
